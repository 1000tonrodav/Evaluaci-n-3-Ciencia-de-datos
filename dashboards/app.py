...
# --- FUNCIONES DE CARGA CON CACHÉ GLOBAL ---
@st.cache_data
...
if df_f1.empty or df_f3.empty:
    ...
    # PESTAÑA 1: REFACTORIZADA CON TRIPLE BENCHMARK CONCEPTUAL
    with tab1:
        ...
        presupuesto_institucional_total = num_empleados_filtrados * 1500  # Meta Empresa: Max $1500 USD pérdida por persona/año
        ...
        meta_acumulada_dias = num_empleados_filtrados * 5  # Meta Empresa: Max 5 días acumulados por persona
        ...
        ratio_ausentismo_institucional = (avg_ausentismo / 5.0) * 100  # Meta Empresa: 5 días ind.
        
        avg_prod = df_filtrado['productivity_score'].mean() if not df_filtrado.empty else 0
        ratio_prod_institucional = (avg_prod / 75.0) * 100  # Meta Empresa: 75% eficiencia
...
- Ideal (Estudios Gartner): 85.0% de rendimiento óptimo continuo sin sobrecarga."""
    ...
    # PESTAÑA 2: RETENCIÓN Y DETALLE DE POLÍTICAS CON TRIPLE BENCHMARK (CORREGIDA Y COMPLETA)
    with tab2:
        ...
        ratio_fuga_institucional = (porcentaje_fuga / 15.0) * 100  # Meta Empresa: Max 15% rotación
        ...
        ratio_cobertura_inst = (cobertura_real / 50.0) * 100  # Meta Empresa: 50% de cobertura mínima
        
        # 3. Tasa de Uso del PAE
        uso_pae_real = (df_filtrado['used_eap'] == 'yes').mean() * 100 if not df_filtrado.empty else 0
        ratio_pae_inst = (uso_pae_real / 10.0) * 100  # Meta Empresa: 10% de uso proyectado óptimo
        ...
        ratio_jefes_institucional = (avg_jefaturas / 4.0) * 100  # Meta Empresa: Nota 4.0 mínimo
...
- Ideal (Estudios Gartner): Cobertura proactiva superior al 70% de la fuerza laboral."""
        ...
        with k7: 
            st.metric(
                ...
                f"{uso_pae_real:.1f}%",
                delta=f"🎯 {ratio_pae_inst:.1f}% del uso óptimo",
                delta_color="normal",
...
- Institucional (Meta Empresa): Tasa de uso óptima proyectada del 10% para contención oportuna.
